...
async function getPolicyData() {
  try {
    const setting = await db.siteSetting.findUnique({
      where: { key: 'policy_delivery' },
    });
    if (setting) {
      return JSON.parse(setting.value);
    }
  } catch (error) {
    console.error('Failed to get policy_delivery:', error);
  }
  return {
    title: 'Delivery Policy',
    content: '<p>Our project delivery milestones and deployment policy details will appear here soon.</p>',
